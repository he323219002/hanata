...
<script>
  import {picBaseURL, postRequest, uploadRequest} from "../../utils/api";
  import {checkToken, getToken} from "../../utils/utils";
  ...
  export default {
    ...
    methods: {
      ...
      async submitDairy() {
        let {data: res} = await postRequest('dairy/new', this.dairyForm);
        if (res.code === 0) {
      ...
      async getArticleById() {
        ...
        if (res.code === 0) {
          ...
          this.dairyForm.tid = Boolean(res.data.tagId) ? res.data.tagId : ''
